...
use ffmpeg_sys_next::{
    ...
    avformat_open_input, avio_alloc_context, avio_context_free, avio_open, AVCodec, AVCodecID,
    AVColorRange, AVColorSpace, AVFilterContext, AVFilterInOut, AVFilterPad, AVFormatContext,
    ...
    AVFMT_NOFILE, AVFMT_NOGENSEARCH, AVFMT_NOSTREAMS, AVIO_FLAG_WRITE, AVSEEK_FLAG_BACKWARD,
...
#[cfg(feature = "docs-rs")]
unsafe fn open_input_file(index: usize, input: &mut Input) -> Result<Demuxer> {
    Err(Bug)
}

#[cfg(not(feature = "docs-rs"))]
pub(crate) unsafe fn open_input_file(
    index: usize,
    input: &mut Input,
    copy_ts: bool,
) -> Result<Demuxer> {
